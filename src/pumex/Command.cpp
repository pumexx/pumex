...
CommandBuffer::CommandBuffer(VkCommandBufferLevel bf, std::shared_ptr<Device> d, std::shared_ptr<CommandPool> cp)
  : bufferLevel{ bf }, commandPool{ cp }, device{d->device}
{
  ...
  VK_CHECK_LOG_THROW(vkAllocateCommandBuffers(device, &cmdBufAllocateInfo, &commandBuffer), "failed vkAllocateCommandBuffers");
}

CommandBuffer::~CommandBuffer()
{
  vkFreeCommandBuffers(device, commandPool->getHandle(device),1, &commandBuffer);
}
...
void CommandBuffer::cmdBegin(VkCommandBufferUsageFlags usageFlags) const
{
  ...
  VK_CHECK_LOG_THROW(vkBeginCommandBuffer(commandBuffer, &cmdBufInfo), "failed vkBeginCommandBuffer");
}

void CommandBuffer::cmdEnd() const
{
  VK_CHECK_LOG_THROW(vkEndCommandBuffer(commandBuffer), "failed vkEndCommandBuffer");
}
...
void CommandBuffer::cmdPipelineBarrier(VkPipelineStageFlagBits srcStageMask, VkPipelineStageFlagBits dstStageMask, VkDependencyFlags dependencyFlags, const PipelineBarrier& barrier) const
{
  ...
  {
  case PipelineBarrier::Memory:
    vkCmdPipelineBarrier(commandBuffer, srcStageMask, dstStageMask, dependencyFlags, 1, &barrier.memoryBarrier, 0, nullptr, 0, nullptr);
    break;
  case PipelineBarrier::Buffer:
    vkCmdPipelineBarrier(commandBuffer, srcStageMask, dstStageMask, dependencyFlags, 0, nullptr, 1, &barrier.bufferBarrier, 0, nullptr);
    break;
  case PipelineBarrier::Image:
    vkCmdPipelineBarrier(commandBuffer, srcStageMask, dstStageMask, dependencyFlags, 0, nullptr, 0, nullptr, 1, &barrier.imageBarrier );
    break;
  }
}
...
{
  vkCmdCopyBuffer(commandBuffer, srcBuffer, dstBuffer, bufferCopy.size(), bufferCopy.data());
}
...
{
  vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, pipeline->getHandle(device));
}

void CommandBuffer::cmdBindPipeline(std::shared_ptr<pumex::GraphicsPipeline> pipeline) const
{
  vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline->getHandle(device));
}
...
void CommandBuffer::cmdBindDescriptorSets(VkPipelineBindPoint bindPoint, std::shared_ptr<pumex::PipelineLayout> pipelineLayout, uint32_t firstSet, const std::vector<std::shared_ptr<pumex::DescriptorSet>> descriptorSets) const
{
  ...
  vkCmdBindDescriptorSets(commandBuffer, bindPoint, pipelineLayout->getHandle(device), firstSet, descSets.size(), descSets.data(), 0, nullptr);
}
...
void CommandBuffer::cmdBindDescriptorSets(VkPipelineBindPoint bindPoint, std::shared_ptr<pumex::PipelineLayout> pipelineLayout, uint32_t firstSet, std::shared_ptr<pumex::DescriptorSet> descriptorSet) const
{
  ...
  // TODO : dynamic offset counts
  vkCmdBindDescriptorSets(commandBuffer, bindPoint, pipelineLayout->getHandle(device), firstSet, 1, &descSet, 0, nullptr);
}
